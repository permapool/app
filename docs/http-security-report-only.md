...
Polling pauses while hidden and refreshes immediately on visibility. A viewer's abort cancels only that browser's same-origin request; it does not cancel provider work shared by other viewers, which is bounded by the independent server deadline. An initial status failure shows concise, noninteractive error text and retries automatically on the same 15-second visible-page schedule; it remains an error until Livepeer authoritatively reports live or offline. The stable live region is not rewritten for each background attempt. A later transient failure retains the last known live or offline presentation and continues polling. Offline state never mounts the HLS player, preventing the previous repeated offline-manifest requests.
