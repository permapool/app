...
- The application-wide `AuthProvider` session synchronization now aborts abandoned same-origin requests and uses monotonic operation ownership so Strict Mode replay, unmounts, logout, rapid identity changes, and older completions cannot commit stale user or loading state. Logout hides application user state immediately, while later logins retain the existing `/api/auth/session` contract and generic failure behavior.
