...
- The application-wide `AuthProvider` session synchronization now aborts abandoned same-origin requests and requires both monotonic operation ownership and the current committed Privy readiness/authentication/identity snapshot at every state boundary. Strict Mode replay, unmounts, logout, readiness loss, rapid identity changes, and older completions therefore cannot commit stale user or loading state. A not-ready Privy session is exposed as loading rather than usable, logout hides application user state immediately, and later ready/login transitions retain the existing `/api/auth/session` contract and generic failure behavior.
