...
- Duplicate page-local Privy provider roots were identified around navigation, the homepage, and the profile page. The required architecture is one stable application-wide `PrivyProvider` → `WagmiProvider` → application `AuthProvider` tree at the root client boundary, with the homepage Liveblocks room remaining an independent descendant.
- The duplicate roots have been consolidated without changing login methods, wallet UI settings, chain configuration, session synchronization, or wallet-link/logout behavior. A one-off Privy iframe-ready queue error observed during clean staging testing must still be retested after publication with login, navigation, logout, wallet-link initiation, and repeated refreshes; the repository does not suppress or patch the SDK error.
- The application-wide `AuthProvider` session synchronization now aborts abandoned same-origin requests and requires both monotonic operation ownership and the current committed Privy readiness/authentication/identity snapshot at every state boundary. Its public user/loading presentation is one atomic record tagged with the exact authentication generation that settled it, so repeated identities cannot reuse an older generation's user state. Strict Mode replay, unmounts, logout, readiness loss, rapid identity changes, and older completions therefore cannot commit or expose stale state. A not-ready or generation-mismatched Privy session is exposed as loading rather than usable, logout hides application user state immediately, and later ready/login transitions retain the existing `/api/auth/session` contract and generic failure behavior.
...
- Farcaster login remains excluded from this slice, and activated server wallets remain unrelated and unused. Wallet synchronization and schema correction are the next implementation step after the consolidated provider tree is verified on staging.
